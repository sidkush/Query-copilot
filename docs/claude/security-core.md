...
- **`transpile_failure` alert fires on sqlglot exception, not on identical output** — `waterfall_router._transpile_for_live_tier` uses `dialect_bridge.transpile_checked()` which returns a `failed` bool. Never compare `out == sql` to detect failure; that's a false positive when identity transpile is valid (e.g. `SELECT 1` bigquery→postgres). `DIALECT_BRIDGE_ALERT_ON_FAILURE` keeps this on by default.
