...
class AlertManager:
    ...
    def _dispatch(self, signal: AlertSignal) -> DispatchResult:
        ...
        webhook = self._webhook_for(signal.tenant_id)
        email = self._email_for(signal.tenant_id)
        disp = SlackDispatcher(
            webhook_url=webhook,
            max_retry=self._max_retry,
            email_fallback=settings.ALERT_FALLBACK_EMAIL_ON_SLACK_FAIL,
        )
        ok = disp.send(payload, recipient_email=email)
        logger.info("alert_dispatch rule=%s tenant=%s ok=%s", signal.rule_id, signal.tenant_id, ok)
        return DispatchResult(ok, "slack" if webhook else "email", None if ok else "dispatch_failed")

    def _webhook_for(self, tenant_id: str) -> str:
        from config import settings
        return getattr(settings, "SLACK_WEBHOOK_URL", "") or ""

    def _email_for(self, tenant_id: str) -> str:
        try:
            from user_storage import get_admin_email_for_tenant
            return get_admin_email_for_tenant(tenant_id) or ""
        except Exception:
            return ""
