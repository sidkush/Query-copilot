    ...
    def to_dict(self) -> dict:
        ...
        if self.data is not None:
            ...
            if hasattr(self.data, "to_dict"):
                ...
                # Lazy-import chart_hints to match the pandas-DLL workaround.
                try:
                    from chart_hints import build_chart_hints, empty_hints
                    result["chart_hints"] = build_chart_hints(df, self.row_count)
                except Exception:
                    from chart_hints import empty_hints
                    result["chart_hints"] = empty_hints(self.row_count)
            else:
                result["rows"] = []
                result["column_profile"] = []
                from chart_hints import empty_hints
