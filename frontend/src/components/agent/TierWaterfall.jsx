...
export default function TierWaterfall({ step }) {
  ...
  return (
    <div style={{ padding: '8px 12px' }} role="group" aria-label="Intelligence tiers">
      ...
      <ul className="flex flex-col gap-1" style={{ listStyle: 'none', padding: 0, margin: 0 }}>
        {TIERS.map((tier, i) => {
          ...
          const statusText = isHit ? 'hit' : checked ? 'miss' : 'pending';
          return (
            <motion.li
              key={tier.key}
              className="flex items-center gap-2 text-xs"
              aria-label={`${tier.label}: ${statusText}`}
            ...
            >
              <span
                aria-hidden="true"
                style={{ width: 8, height: 8, borderRadius: '50%', background: isHit ? tier.color : checked ? `${tier.color}40` : TOKENS.border.default }}
              />
