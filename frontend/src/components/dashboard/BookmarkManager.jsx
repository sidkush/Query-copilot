...
export default function BookmarkManager({ dashboardId, currentState, onApply, onClose }) {
  ...
  return (
    <AnimatePresence>
      <motion.div key="bm-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <motion.div key="bm-modal" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px 20px' }}>
            ...
            {bookmarks.map(bm => (
              <div key={bm.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: `1px solid ${TOKENS.border.default}` }}>
