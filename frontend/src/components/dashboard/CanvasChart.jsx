...
export default function CanvasChart({ columns, rows, chartType = 'scatter', formatting = null }) {
  ...
  const cc = useMemo(() => resolvedTheme === 'light'
    ? { axis: '#6b7280', axisLine: '#d1d5db', grid: '#e5e7eb', gridDashed: '#d1d5db', tooltipBg: '#ffffff', tooltipBorder: '#d1d5db', tooltipText: '#1f2937', legendText: '#6b7280', visualMapLow: '#e5e7eb' }
    : { axis: '#94a3b8', axisLine: '#1e293b', grid: '#1e293b', gridDashed: '#162032', tooltipBg: '#0f172a', tooltipBorder: '#334155', tooltipText: '#e2e8f0', legendText: '#9ca3af', visualMapLow: '#162032' },
  [resolvedTheme]);
  ...
  const option = useMemo(() => {
    ...
    if (chartType === 'scatter' && numericCols.length >= 2) {
      return {
        ...
        xAxis: {
          type: 'value',
          name: numericCols[0],
          nameTextStyle: { color: cc.axis, fontSize: 11 },
          axisLabel: { color: cc.axis, fontSize: 11 },
          axisLine: { lineStyle: { color: cc.axisLine } },
          splitLine: { lineStyle: { color: cc.gridDashed, type: 'dashed' } },
        },
        yAxis: {
          type: 'value',
          name: numericCols[1],
    ...
    if (chartType === 'heatmap') {
      ...
      return {
        backgroundColor: 'transparent',
        tooltip: { position: 'top', backgroundColor: cc.tooltipBg, borderColor: cc.tooltipBorder, textStyle: { color: cc.tooltipText, fontSize: 12 } },
        grid: { left: 80, right: 30, top: 20, bottom: 60 },
        xAxis: { type: 'category', data: xValues, axisLabel: { color: cc.axis, fontSize: 10, rotate: xValues.length > 20 ? 45 : 0 }, axisLine: { lineStyle: { color: cc.axisLine } } },
        yAxis: { type: 'category', data: yValues, axisLabel: { color: cc.axis, fontSize: 10 }, axisLine: { lineStyle: { color: cc.axisLine } } },
        visualMap: { min: minVal, max: maxVal, calculable: true, orient: 'horizontal', left: 'center', bottom: 0, inRange: { color: [cc.visualMapLow, palette[0]] }, textStyle: { color: cc.axis } },
    ...
    return {
      ...
      xAxis: { type: 'category', data: rows.map(r => r[labelCol]), axisLabel: { color: cc.axis, fontSize: 10 }, axisLine: { lineStyle: { color: cc.axisLine } } },
      yAxis: { type: 'value', axisLabel: { color: cc.axis, fontSize: 11 }, axisLine: { lineStyle: { color: cc.axisLine } }, splitLine: { lineStyle: { color: cc.gridDashed, type: 'dashed' } } },
      ...
      legend: { textStyle: { color: cc.legendText, fontSize: 11 }, bottom: 0 },
    };
  }, [columns, rows, chartType, formatting, palette, cc]);
